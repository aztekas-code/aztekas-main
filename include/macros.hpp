 ...
 *
 *  @brief    Macro definitions for the aztekas code.
 *
 *  @details
 *  ---------------------------------------------------------------------------------
 *  This header defines a collection of macros used throughout the aztekas codebase:
 *    - Min/Max functions for 2 or 4 arguments.
 *    - Logical flags and simple physical model flags.
 *    - Macros to handle array indexing depending on the dimension (DIM).
 *    - Macros that expand to indexing expressions for multi-dimensional arrays.
 *
 *  IMPORTANT NOTES:
 *  ---------------------------------------------------------------------------------
 *  - These macros are in C style. In modern C++, inline or template functions are
 *    often preferred over macros for type safety and debugging convenience.
 *  - The include guard starts with "INCLUDE_AZTEKAS_" and ends with "_", as requested.
 *  - Ensure that DIM and eq are defined appropriately before including this header.
 *
 ************************************************************************************/
...
/**
 * @brief Four-argument minimum.
 *        Returns the minimum of (a, b, c, d).
 */
#define Min(a, b, c, d)  ( (a < b ? a : b) < (c < d ? c : d) ? \
                        (a < b ? a : b) : (c < d ? c : d) )

/**
 * @brief Four-argument maximum.
 *        Returns the maximum of (a, b, c, d).
 */
#define Max(a, b, c, d)  ( (a > b ? a : b) > (c > d ? c : d) ? \
                        (a > b ? a : b) : (c > d ? c : d) )
...
/* SIMPLE ARRAY (likely used for matrix A) */
#define a(x, y) a[(x)*(eq) + (y)]

/**
 * @brief Macros to index arrays depending on the dimension (DIM).
 *
 * These macros assume that Nx1, Nx2, Nx3, and eq are defined, and that the
 * arrays in question are allocated/defined properly elsewhere. They expand
 * into the correct indexing for 1D, 2D, or 3D data.
 *
 * DIM == 1 => 1D expansions
 * DIM == 2 or 4 => 2D expansions
 * DIM == 3 => 3D expansions
 */
#if DIM == 1
