 ...
 * Description : aztekas user input parameters for Dust Accretion
 * Creation Date : 27-09-2019
 * Last Modified : 27-09-2019 00:27:20
   ...
   while(fscanf(paramfile, " %c", &t_firstChar) == 1 )
   {
      ...
      if(t_firstChar != '/' && t_firstChar != '%')
      {
         ...
         if(fscanf (paramfile, "%s = %s", t_key, t_value) == 2)
         {
            ...
            if(strcmp(t_key,"r_dot_0")==0)
            {
               r_dot_0 = atof(t_value);
            }

            if(strcmp(t_key,"phi_dot_0")==0)
            {
               phi_dot_0 = atof(t_value);
            }
         }
      }
