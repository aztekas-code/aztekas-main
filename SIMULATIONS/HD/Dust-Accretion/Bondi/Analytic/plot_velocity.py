...
#######################################################
#
# Plot velocity from simulation data
...
# set cutoff for maximum velocity
v_max = .3
...
plt.contour(xx, zz, vel, 20, linewidths=0.5, colors='k')
CS = plt.contourf(xx, zz, vel, 100, cmap=cm, extend='max')

#vmin=0.1,vmax=2.0,
#CS.cmap.set_under('white')

cbar=plt.colorbar()
cbar.set_label('$|v/c_o|$', labelpad=-40, y=1.06, rotation=0,fontsize=fs)

cbar.ax.tick_params(labelsize=fs)

#plt.clim(0.1,2.0)

plt.streamplot(xx,zz,vvx,vvz,density=1,color='w',linewidth=1.5)
...
plt.savefig('velocity_'+out+'.pdf', bbox_inches='tight', pad_inches=0.1)
