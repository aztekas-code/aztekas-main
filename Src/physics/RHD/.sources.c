...
//   gam[2][2] = local_grid->gamma_con[2][2];
   ...
   for(int i = 0; i < 3; i++)
   {
      beta[i] = local_grid->beta_con[i];
      Su[i] = S_con[i];
      Sd[i] = S_cov[i];
      for(int j = 0; j < 3; j++)
      {
         gam[i][j] = local_grid->gamma_con[i][j];
         W[i][j]   = Su[i]*v_con[j] + p*gam[i][j];
         if(i == j)
         {
            Wd[i][j]  = Sd[i]*v_con[j] + p; 
         }
         else
         {
            Wd[i][j]  = Sd[i]*v_con[j]; 
...
//   Wik_dgamik[2] = W[0][0]*der.dgam[2][0][0] + W[0][1]*der.dgam[2][0][1] + W[0][2]*der.dgam[2][0][2] + \
   ...
   for(int j = 0; j < 3; j++)
   {
      ...
      for(int i = 0; i < 3; i++)
      {
         for(int k = 0; k < 3; k++)
         {
            Wik_dgamik[j] += W[i][k]*der.dgam[j][i][k];
         }
      }
   }
...
//   Wik_betaj_dgamjik = Wik_dgamik[0]*beta[0] + Wik_dgamik[1]*beta[1] + Wik_dgamik[2]*beta[2];
   ...
   Wik_betaj_dgamjik = 0.0;
   for(int i = 0; i < 3; i++)
   {
      Wik_betaj_dgamjik += Wik_dgamik[i]*beta[i];
   }
...
//   Wdij_dbetaji = Wd[0][0]*der.dbeta[0][0] + Wd[0][1]*der.dbeta[0][1] + Wd[0][2]*der.dbeta[0][2] + \
   ...
   for(int i = 0; i < 3; i++)
   {
      for(int j = 0; j < 3; j++)
      {
         Wdij_dbetaji += Wd[i][j]*der.dbeta[j][i];
      }
   }
